...
import { X, Send, Bot, User, Sparkles, Loader2, Phone } from 'lucide-react';
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'bot',
      text: 'Halo! Saya Asisten AI NKExpress 🚢 Ada yang ingin Anda tanyakan tentang tarif, jadwal pengiriman, alamat transit, atau cara jastip rute Ternate - Sofifi & Maluku Utara?',
    },
  ]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const botReply: Message = {
        ...
        sender: 'bot',
        text: data.reply || 'Terima kasih telah bertanya! Silakan konsultasikan langsung dengan CS WhatsApp di 08215046568.',
      };
      ...
      const errorMsg: Message = {
        id: (Date.now() + 1).toString(),
        sender: 'bot',
        text: 'Maaf, sambungan internet terputus. Silakan hubungi CS WhatsApp kami di 08215046568.',
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md h-[580px] flex flex-col overflow-hidden text-left shadow-2xl relative">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-extrabold text-slate-900 flex items-center gap-1.5">
                <span>Asisten AI NKExpress</span>
                <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              </h3>
              <p className="text-[10px] text-emerald-600 font-medium">Online • Siap membantu 24/7</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-3 py-2 bg-slate-100 border-t border-slate-200 flex gap-1.5 overflow-x-auto text-[10px] whitespace-nowrap">
          <button
            onClick={() => handleQuickQuestion('Berapa ongkir ke Ternate?')}
          ...
          <button
            onClick={() => handleQuickQuestion('Di mana alamat gudang Ternate?')}
            className="px-2.5 py-1 rounded-full bg-white hover:bg-slate-200 text-slate-700 border border-slate-300 cursor-pointer shadow-2xs"
          >
            Alamat Gudang
          </button>
          <button
            onClick={() => handleQuickQuestion('Bagaimana cara jastip Shopee?')}
            className="px-2.5 py-1 rounded-full bg-white hover:bg-slate-200 text-slate-700 border border-slate-300 cursor-pointer shadow-2xs"
          >
            Cara Jastip
          </button>
